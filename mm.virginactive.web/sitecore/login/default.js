...
function scToggleOptions() {
    ...
    scToggleElement('options-text');
    return false;
}

function scToggleElement(target) {
    var e = document.getElementById(target);
    var isHidden = e.style.display == "none";
    e.style.display = isHidden ? "" : "none";
    if ( $(".chosen-select", $('#LoginPanel')).length !== 0  ) {
        if (e.style.display === 'none') {
            $('#LoginPanel').addClass('options-open');
            $(".chosen-select", $('#LanguagePanel')).chosen();
        } else {
            $('#LoginPanel').removeClass('options-open');
        }
    }
    return false;
}
