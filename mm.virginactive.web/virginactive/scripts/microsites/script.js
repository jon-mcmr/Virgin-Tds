...
//				});
	...
	var homepage_carousel = function(){
		...
		var togglePause = function(){
			if(privateProperties.paused === true){
				privateProperties.paused = false;
				privateProperties.timer = setInterval(rotatePanels, 5000);
			}
			else{
				privateProperties.paused = true;
		...
		init: function(){
			...
			if(($('html').hasClass('no-touch') || (navigator.userAgent.indexOf('iPhone') === -1))){
                ...
                $(".lightbox").colorbox({ 
                    ...
                    onOpen: homepage_carousel.togglePause,
                    onClosed: homepage_carousel.togglePause
                });
			}
