$(function () {
    ...
    if ($html.find("article").parent().hasClass("wrapper-thanks")) {
        $("body").addClass("body-thanks");
    }

    //Add tooltip on timetable for class info and "token" copy
    var tooltip = $('.tooltip em');
    $(tooltip).hide();

    $('.tooltip').hover(function () {
        $(this).find(tooltip).stop(true, true).fadeToggle();
    });

    //Add movement to arrow on ballow box
    $enter.wrapInner("<a href='#' />");
    $("#ballotbox").hover(function () {
        $(this).addClass("hover").children("#arrow").stop(true, true).animate({ top: "6px" });
    }, function () {
        $(this).removeClass("hover").children("#arrow").stop(true, true).animate({ top: "-4px" });
    });

    $("#ballotbox").click(function () {
        $("html,body").animate({ scrollTop: "538px" }, 2000, "easeOutQuint");
    });

    //Switch "Enter the ballot" with image replacement if <ie9
    ...
    function updateValidationData() {
        elems = [[$form.find(".text-firstname"), "^[a-zA-Z-' ]+$"],
					...
					[$form.find(".text-address"), "^[a-zA-Z0-9- ]+$"],
					[$form.find(".text-town"), "^[a-zA-Z- ]+$"],
					[$form.find(".text-postcode"), "^[a-zA-Z0-9 ]+$"],
					[$form.find(".text-number"), "^[0-9 ()]+$"],
					[$form.find(".text-dob"), "^(([0-9])|([0-2][0-9])|(3[0-1]))\/(([1-9])|(0[1-9])|(1[0-2]))\/(([0-9][0-9])|([1-2][0,9][0-9][0-9]))$"],
					[$form.find(".text-kin"), "^[a-zA-Z-' ]+$"],
					[$form.find(".text-kinnumber"), "^[0-9 ()]+$"]];
    ...
    function validateElems($el, eventHandler) {
        $el.bind(eventHandler, function () {
            var $this = $(this),
				...
				clubGUID = $form.find(".chzn-clublist option:selected").val();
        ...
        if (count > 0) {
            ...
            if (!hasSubmitted) {
                ...
                if (typeof (_gaq) !== "undefined") {
                    _gaq.push(["_trackEvent", "Form", "Submit", "Campaign-LondonTriathlonBallot"]);
                }
